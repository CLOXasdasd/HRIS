...
<?php
	...
	function generateRow($id) {
		...
		$query = $conn->query($sqlLeaveEmployee);s
	...
	function generateRowDetails($id) {
		...
		$sql = "SELECT leave_id,leaveRequest,reason,startDate, endDate, days FROM hirs_db.tbl_leaverequest WHERE emp_id = '$id' and status = 'Approved' AND startDate like '$year%' ORDER BY leaveRequest DESC";
